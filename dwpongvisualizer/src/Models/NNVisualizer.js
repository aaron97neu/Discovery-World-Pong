...
function Neurons (props) {
  ...
  useFrame((clock) => {
    neuron_refs.map((neuron, idx) => {
      // console.log(`neuron: ${ JSON.stringify(neuron)}`);
      console.log(`idx: ${idx}`);
      if (current_activations != null && idx > 0) {
        console.log(`current_activations[idx]: ${current_activations[idx]}`);
