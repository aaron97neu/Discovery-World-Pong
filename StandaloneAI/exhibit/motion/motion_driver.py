...
class MotionDriver:
    ...
    def motion_loop(self):
        ...
        while True:
            ...
            print(f'human position {position}')
            self.subscriber.publish("motion/position", {"position": (position)})
            #if position == 0.5:
                # position is our dummy value for not seeing a human blob
                # continue loop back at waiting for interaction
            #    has_bad_values = True

            if loopCounter < 50:
                arrayVals = np.append(arrayVals, position)
                loopCounter += 1
            else:
                # if there is a small deviation or the value is unchanged after 50 samples
                if np.std(arrayVals) > 0.06 or np.average(arrayVals) == position:
        ...
        while True:
            ...
            if(self.subscriber.game_state == 0): # waiting for confirmation of player
                ...
                for counter in range(0,50):
                    ...
                    self.subscriber.publish("motion/position", {"position": (c_value)})
                    print(f'human position {c_value}')
                    if c_value == 0.5:
