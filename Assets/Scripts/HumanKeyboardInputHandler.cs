...
public class HumanKeyboardInputHandler : MonoBehaviour
{
    [Tooltip("Option to use depth camera as player input. Will use keyboard if false.")]
    public bool playerUseDepthCamera = true;
    ...
    */
    ...
    private void OnMessageArrivedHandler(string newMsg)
    {
         ...
         CameraPosition inputPosition = JsonConvert.DeserializeObject<CameraPosition>(newMsg);
         float inputXPosition = inputPosition.position;
        //float inputXPosition = float.Parse(newMsg);

        float slope = 1.0f * (unityMaxPosition - unityMinPosition) / (depthMaxPosition - depthMinPosition);

        //normalized to gameboard x position
        float newXPosition = unityMinPosition + slope * (inputXPosition - depthMinPosition);
        //playerPosition = unityMinPosition + slope * (paddlePosition.position - depthMinPosition);
        //Debug.Log("MQTT position " + newMsgJson.position + " | playerPosition " + playerPosition);
        //playerPosition = ((float.Parse(newMsgJson.position) + 0.675f) * 9.8181818f);
        Vector3 currentPosition = myTransform.position;
        Debug.Log("Depth camera position " + newXPosition + " | " + (Mathf.Clamp(newXPosition - currentPosition.x, -1, 1)));

        //float newSpeed = 0.0f;
        float deltaX = (Mathf.Clamp(newXPosition - currentPosition.x, -1, 1)) * speed;

        newXPosition = currentPosition.x + deltaX;
        newXPosition = Mathf.Clamp(newXPosition, -maxOffset, maxOffset);
