...
public class GameStateToMQTT : MonoBehaviour  // : M2MqttUnityClient //
{
    ...
    {
        ;
        Transform topPlayerTransform = GameObject.Find("ComputerPaddle").GetComponent<Transform>();
        Transform bottomPlayerTransform = GameObject.Find("HumanPaddle").GetComponent<Transform>();
        ...
        if (_eventSender.isConnected)
        {
            ...
            _eventSender.Publish("game/frame", JsonConvert.SerializeObject(new { frame = Time.frameCount })); //string.Join("", Time.frameCount)
            _eventSender.Publish("paddle1/position", bottomPlayerPayload); // computer subscribes to this information
        }
    }
}
